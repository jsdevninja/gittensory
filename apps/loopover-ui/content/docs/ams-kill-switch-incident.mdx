...
4. **Automated PagerDuty page** (#7666) — when `LOOPOVER_ENABLE_PAGERDUTY` is on and a routing key
   is configured, a kill-switch **trip** (engage) fires an Events API v2 page via the same path ORB
   uses (`notify-pagerduty`). A resume does not page. Treat an `ams_kill_switch:*` incident as
   "operator already needed on the page" and start the 15-minute response clock immediately.
