// Generic Sentry release/deploy verification, mirroring review-enrichment/scripts/validate-sentry-release.mjs
// (org/project-agnostic -- adapted here only by dropping that copy's Railway-specific env-var fallbacks,
// since discovery-index deploys via Cloudflare Containers, not Railway).
...
export function loadSentryReleaseValidationConfig(env = process.env) {
  return {
    ...
    expectedCommitSha: nonBlank(env.SENTRY_EXPECT_COMMIT_SHA) ?? nonBlank(env.SENTRY_COMMIT_SHA),
    expectedDeployName: nonBlank(env.SENTRY_DEPLOY_NAME),
    expectedEnvironment: nonBlank(env.SENTRY_ENVIRONMENT) ?? "production",
...
function requireConfig(config) {
  const missing = [
    ...
    .map(([name]) => name);
  if (missing.length > 0) {
    throw new SentryReleaseValidationError("missing Sentry release validation config", [`missing ${missing.join(", ")}`]);
  }
}
...
async function sentryJson(config, segments, fetchImpl) {
  ...
  if (!response.ok) {
    ...
    throw new SentryReleaseValidationError("Sentry API request failed", [`${segments.join("/")} returned HTTP ${response.status}${message ? ` (${message})` : ""}`]);
  }
...
function releaseProjects(release) {
  return asArray(release?.projects).map((project) => stringField(project, ["slug", "name"])).filter(Boolean);
}
...
export async function validateSentryRelease(env = process.env, fetchImpl = globalThis.fetch) {
  ...
  const release = await sentryJson(config, ["organizations", config.org, "releases", config.release], fetchImpl);
  ...
  if (config.requireCommits) {
    commits = asArray(await sentryJson(config, ["organizations", config.org, "releases", config.release, "commits"], fetchImpl));
    const commitCount = typeof release?.commitCount === "number" ? release.commitCount : commits.length;
    const commitIds = [...commitIdsFrom(release), ...commits.flatMap((commit) => commitIdsFrom(commit))];
  ...
  if (config.requireDeploy) {
    deploys = asArray(await sentryJson(config, ["organizations", config.org, "releases", config.release, "deploys"], fetchImpl));
    const deployCount = typeof release?.deployCount === "number" ? release.deployCount : deploys.length;
    ...
    if (deployCount <= 0 && allDeploys.length === 0) {
      ...
      failures.push(`release deploys do not include ${config.expectedEnvironment}/${config.expectedDeployName ?? "any"}`);
    }
  }

  let releaseFiles = [];
  if (config.requireReleaseFiles) {
    releaseFiles = asArray(await sentryJson(config, ["projects", config.org, config.project, "releases", config.release, "files"], fetchImpl));
...
async function main() {
  try {
    ...
    log("discovery_index_sentry_release_validation_complete", result);
  } catch (error) {
    const failures = Array.isArray(error?.failures) ? error.failures : [String(error)];
    logError("discovery_index_sentry_release_validation_failed", { release: nonBlank(process.env.SENTRY_RELEASE), failures });
    process.exitCode = 1;
  }
}
