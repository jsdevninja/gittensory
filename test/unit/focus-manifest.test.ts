...
describe(".loopover.yml.example field-exhaustiveness (#1670)", () => {
  ...
  const GATE_FIELD_TOKENS = {
    ...
    contentLaneDeliverable: "contentLaneDeliverable:",
    backtestRegression: "backtestRegression:",
    dryRun: "dryRun:",
  ...
  const SETTINGS_FIELD_TOKENS = {
    ...
    issuePlanExtraLabels: "issuePlanExtraLabels:",
    issuePlanMilestoneReuse: "issuePlanMilestoneReuse:",
  ...
  const FEATURES_FIELD_TOKENS = {
    ...
    amsReputationBridge: "amsReputationBridge:",
...
describe("parseFocusManifest settings override + resolveEffectiveSettings", () => {
  ...
  it("wires settings.advisoryAiRouting.chatQaFrontierFallback into the manifest parser as a sparse override (#4595 follow-up)", () => {
    const parsed = parseFocusManifest({ settings: { advisoryAiRouting: { chatQaFrontierFallback: true } } });
    expect(parsed.settings.advisoryAiRouting).toEqual({ chatQaFrontierFallback: true });
    expect(parsed.warnings).toEqual([]);
  });
