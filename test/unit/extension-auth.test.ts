...
describe("extension auth storage", () => {
  it("stores session tokens only in extension local storage and keeps sync storage token-free", async () => {
    const storage = fakeExtensionStorage({
      sync: { apiOrigin: "https://loopover-api.aethereal.dev/ignored/path", sessionToken: "legacy-sync-token" },
    });

    await saveExtensionApiOrigin("https://api.loopover.test/v1", storage);
    ...
    expect(session).toMatchObject({
      apiOrigin: "https://api.loopover.test",
    ...
    expect(storage.sync.dump()).toEqual({ apiOrigin: "https://api.loopover.test" });
  ...
  it("rejects GitHub personal access tokens and malformed tokens before storage", async () => {
    ...
    await expect(storeExtensionSessionToken({ token: "not-a-loopover-session" }, storage)).rejects.toThrow(/gts_/i);
  ...
  it("clears locally expired sessions without calling the API", async () => {
    const storage = fakeExtensionStorage({
      sync: { apiOrigin: "https://api.loopover.test" },
      local: { sessionToken: VALID_TOKEN, sessionExpiresAt: "2020-01-01T00:00:00.000Z" },
    });
    const fetchImpl = vi.fn();

    await expect(requestPullContext({ owner: "JSONbored", repo: "loopover", pullNumber: 148 }, { storage, fetchImpl })).rejects.toThrow(EXTENSION_SESSION_EXPIRED_MESSAGE);
  ...
  it("clears revoked or insufficient-scope sessions returned by the API", async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.loopover.test/v1/extension/pull-context?owner=JSONbored&repo=loopover&pullNumber=148",
  ...
  it("returns public-safe pull context payloads and logs out by revoking then clearing local state", async () => {
    ...
    await expect(requestPullContext({ owner: "JSONbored", repo: "loopover", pullNumber: 148 }, { storage, fetchImpl })).resolves.toMatchObject({
    ...
    expect(fetchImpl).toHaveBeenLastCalledWith(
      "https://api.loopover.test/v1/auth/logout",
