...
describe("openRepoDocPullRequest (#3000)", () => {
  ...
  it("#4613: uses env.PUBLIC_SITE_ORIGIN in the generated AGENTS.md attribution link when configured", async () => {
    const env = createTestEnv({ GITHUB_APP_PRIVATE_KEY: generateRsaPrivateKeyPem(), PUBLIC_SITE_ORIGIN: "https://loopover.example.org" });
    await seedInstalledRepo(env, { defaultBranch: "main" });
    await seedProfileData(env);
    await seedRepoDocGenerationConfig(env, REPO);
    const calls: Array<{ method: string; url: string; body: Record<string, unknown> }> = [];
    ...
    expect(agentsContent).toContain("This file is generated by [LoopOver](https://loopover.example.org)");
