...
describe("signal coverage edge cases", () => {
  ...
  it("buildPublicPrPanelSignalRows derives the gate conclusion across provided/fallback paths (#1007 unified-panel extraction)", () => {
    ...
    const advisory = buildPublicPrPanelSignalRows({ ...baseArgs, settings: { ...repoSettings(directRepo.fullName), reviewCheckMode: "disabled" } });
    ...
    const neutral = buildPublicPrPanelSignalRows({ ...baseArgs, repo: null, settings: { ...repoSettings(directRepo.fullName), reviewCheckMode: "required" } });
    ...
    const blocked = buildPublicPrPanelSignalRows({
      ...
      settings: { ...repoSettings(directRepo.fullName), reviewCheckMode: "required", linkedIssueGateMode: "block" },
    });
    ...
    const duplicateBlocked = buildPublicPrPanelSignalRows({
      ...
      settings: { ...repoSettings(directRepo.fullName), reviewCheckMode: "required", duplicatePrGateMode: "block" },
    });
  ...
  it("#dup-winner: panel hard-duplicate block is suppressed for the winner, kept for the loser, byte-identical when flag OFF", () => {
    ...
    const blockSettings = { ...repoSettings(directRepo.fullName), reviewCheckMode: "required" as const, duplicatePrGateMode: "block" as const };
  ...
  it("#dup-winner: hides duplicate-only same-issue evidence while preserving mixed scoped overlap context", () => {
    ...
    const baseArgs = {
      ...
      settings: { ...repoSettings(directRepo.fullName), reviewCheckMode: "required" as const, duplicatePrGateMode: "block" as const },
